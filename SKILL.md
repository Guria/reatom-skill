...
- **Choose stale-preserve vs clean-scope per route** — route loader status follows `withAsyncData`: `status.data` is cached data and remains the last fulfilled value while the next run is pending. Use `isPending && isEverSettled` for same-identity refreshes such as list filters/search. For identity-changing routes (`:id`, selected detail/edit), prefer returning a fresh `reatom*` scoped model keyed by params before awaiting remote data: empty/null atoms plus an internal `load` action or async computed with status + abort handling. The page then renders the model's own loading/error state instead of temporarily reusing the previous entity.
...
- **Treating refresh status as universal** — `isPending && isEverSettled` is the stale-while-refresh branch for the same page identity. On identity changes it can expose the previous loader payload. Prefer a fresh scoped model for the new params; if you cannot reshape the loader yet, render a loader until `status.data` matches the current params.
