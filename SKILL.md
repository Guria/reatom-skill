...
Boundary callbacks need a frame. UI events, timers, third-party callbacks, promise continuations, and anything scheduled by the host enter a fresh context; wrap handwritten callbacks that touch atoms with `wrap()`. Adapter-generated handlers are usually already wrapped, but manual handlers are not.
