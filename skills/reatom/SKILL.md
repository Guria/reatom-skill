...
- **On the first runtime/browser error, stop feature iteration and switch into the debug path.** Re-read `references/meta/gotchas.md`, then use the sibling `reatom-debug` workflow before making more speculative edits. A green `npm run validate` is not enough if it never exercises the failing runtime path.
...
- **Version-pinned UI library provider/control props must be re-validated from docs or installed types before wiring persisted state to them.** After the first provider/settings integration, prove the controlled behavior in a real browser instead of assuming the atom change is enough.
