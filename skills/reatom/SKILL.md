...
- **Version-pinned UI library provider/control props must be re-validated from docs or installed types before wiring persisted state to them.** After the first provider-controlled integration, prove the controlled behavior in a real browser instead of assuming the atom change is enough.
