...
- Navigation callbacks created in `render(self)` and passed to child components still cross a host callback boundary later. Create them with `wrap(...)` at render time (for example `goDetail={wrap((id) => detailRoute.go({ id }))}`) instead of passing a plain `() => route.go(...)` helper down.
