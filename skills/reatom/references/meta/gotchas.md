...
- Callbacks built in `route.render(self)` and passed down as props still need `wrap(...)` under strict setup. Treat `goList={() => listRoute.go()}` in a route render as the same bug class as any other handwritten unwrapped UI callback.
