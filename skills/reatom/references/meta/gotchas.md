...
- `withAsyncData` overload trap: when using `initState`, let TypeScript infer from `initState` or annotate the value; do not force a generic that selects the no-`initState` overload. `initState` itself is optional, so do not add `null as T | null` by reflex — use a typed constant only when a stable initial `.data()` value is truly part of the contract, and short-circuit nullable prerequisites before the API boundary.
...
- Do not mute or remove logger/debug visibility as the first response to `AbortError` noise. In routing work, repeated abort logs are often the symptom of wrong route ownership; fix ownership first, then decide whether the remaining dev logs are acceptable.
- If auth/session restore code catches around `await wrap(api.me())` or a similar request, rethrow `isAbort(err)` before clearing tokens or session state. Route/navigation aborts are normal cancellation, not proof that the session is invalid.
...
- The root element returned from each route `render` should have a static `key` because parent routes render child outputs as an outlet array. Keep it stable per route; only use params/search in the key when an intentional remount is desired. In React, keyed `<Fragment>` is fine; shorthand `<>...</>` cannot carry the key prop.
- Route paths have no leading `/`; use `reatomRoute('')` for root; `route.go()` takes params (or nothing), not a path string.
- Callbacks built in `route.render(self)` and passed down as props still need `wrap(...)`. Treat `goList={() => listRoute.go()}` in a route render as the same bug class as any other handwritten unwrapped UI callback.
- Common `reatomRoute({ ... })` options do not include a `name` field. Keep the name in the variable or surrounding module structure instead of passing `name` into the route options object.
...
- Treat broad default redirects as a high-risk area: do not ship a `path: ''` or pathless redirect child under a layout until you can point to the exact pathname/ownership check that limits it to its intended URL. Generic routing intuition is not enough here.
- Pathless wrapper routes that redirect are especially risky. If a wrapper exists only to share layout, prefer concrete child pages that render through a shared layout helper instead of letting a broad wrapper own redirect logic.
...
- The same caution applies even when the child is "just the default page" and not only a data loader. A `path: ''` child under a layout can still become the wrong owner for redirects, active state, or route-scoped page wiring. If the child exists only to represent the parent's exact page, consider rendering that exact page directly from the parent route instead.
...
- Prefer `reatomComponent` for greenfield pages/layouts/examples; `useAtom` / `useAction` are valid when matching an existing hook-style codebase. Components that call atom getters must be `reatomComponent`; this includes root components, extracted child/row helpers, and navigation items, not just obvious page components. `useAtom`-based components do not need that wrapper because the hook manages subscription. If you choose hooks, hook results are plain values, not callable atom getters; `Boolean/String/... has no call signatures` usually means a hook result was called like `value()`.
- If Reatom/UI-library integration type errors pile up, stop at the first wrong assumption. Do not apply broad casts, `ts-expect-error`, or search-and-replace edits across JSX to make errors disappear. Those usually convert an API misconception into syntax damage and hidden runtime bugs.
- Do not call `wrap(...)` directly in JSX of a plain function component. `wrap()` captures the current Reatom frame at call time, so create wrapped callbacks inside `reatomComponent` (or another reactive caller), or pass pre-wrapped callbacks down as props. Treat `const handler = wrap(...)` inside a plain component render as the same mistake.
- The first React component that reads atoms or routes must render under `<reatomContext.Provider>` (or another established frame source). If the top-level wrapper both provides the frame and needs Reatom state, split it into a plain provider wrapper and an inner `reatomComponent` root.
...
- Declaring route work "done" without manually checking every sibling URL affected by a new guard/default redirect (for example an unaffected sibling page, the default child page, and a deeper child page).
